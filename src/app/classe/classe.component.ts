...
export class ClasseComponent implements OnInit {
  ...
  action($name:any,$val?:any){
    ...
    if($name=='delete'){
      this.currentindex=$val;
      this.crud.delete(this.urlpost,this.listclass[this.currentindex]._id).subscribe({
        next:(data:any)=>{
  ...
  update(){
    this.crud.put(this.urlpost,this.addclasseForm.value,this.listclass[this.currentindex]._id).subscribe({
      next:(data:any)=>{
